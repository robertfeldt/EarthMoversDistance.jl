 ...
 * @param double *cost : pairwise cost matrix; cost[i + n_x * j] holds cost to
...
double emd(int n_x, double *weight_x,
    ...
    while (1) {
        ...
        while (1) {
            var->color = GRAY;
            if (solved_x[var->row]){
                dual_y[var->col] = (cost[(var->row) + n_x * (var->col)]
                                    - dual_x[var->row]);
                solved_y[var->col] = 1;
            } else if (solved_y[var->col]) {
                dual_x[var->row] = (cost[(var->row) + n_x * (var->col)]
                                    - dual_y[var->col]);
                solved_x[var->row] = 1;
            } else {
                assert(FALSE);
        ...
        for (i = 0; i < n_x; i++) {
            for (j = 0; j < n_y; j++) {
                slack = cost[i + n_x * j] - dual_x[i] - dual_y[j];
    ...
    distance = 0;
    for (i = 0; i < B; i++) {
        distance += (basis[i]->flow * cost[(basis[i]->row) + n_x * (basis[i]->col)]);
    }

    if (flows != NULL) {
        // Initialize to zero
        for (i = 0; i < n_x; i++) {
            for(j = 0; j < n_y; j++) {flows[i + n_x * j]=0.0;};
        }
        for (i = 0; i < B; i++) {
            flows[(basis[i]->row) + n_x * (basis[i]->col)] = basis[i]->flow;
        }
    }
